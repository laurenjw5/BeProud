...
//			String[] passUserInput = entryObject.get(timeStamp).toArray(new String[entryObject.size()]);
			...
			result.putExtra("user's entry to edit", e.getText().toString());
			
			
			Log.v("MainActivity", "entry: " + result.getExtras().getString("user's entry to edit"));
			
//			result.putExtra("user's entry date", timeStamp);
